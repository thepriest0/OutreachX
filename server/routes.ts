...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put('/api/auth/profile', requireAuth, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const { firstName, lastName, company, bio } = req.body;
  ...
  app.get('/api/auth/settings', requireAuth, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      res.json({
        ...
        companyName: settings?.companyName || "Nydl Studio",
        companyWebsite: settings?.companyWebsite || "",
        companyDescription: settings?.companyDescription || "A UI/UX and branding design studio",
        defaultTone: settings?.defaultTone || "professional",
        aiPersonality: settings?.aiPersonality || "Professional, helpful, and results-oriented",
        emailNotifications: settings?.emailNotifications ?? true,
        campaignNotifications: settings?.campaignNotifications ?? true,
        leadNotifications: settings?.leadNotifications ?? true,
  ...
  app.put('/api/auth/settings', requireAuth, async (req: any, res) => {
    try {
      const userId = req.user.id;
